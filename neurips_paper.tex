...
To ensure consistency and rigor across debates, we implemented structured prompts that enforced specific argumentation formats for each speech type. Each debater received prompts requiring:

\begin{itemize}
    \item \textbf{Opening speeches}: Three structured arguments with explicit claims, evidence/principles, and logical connections
    \item \textbf{Rebuttal speeches}: Direct engagement with opponent's claims through quoted responses and counter-evidence
    \item \textbf{Closing speeches}: Synthesis of key clashes and explicit weighing of competing arguments
\end{itemize}
