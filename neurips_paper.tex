...
Self Red-Teaming prompts that explicitly instruct models to consider both winning and losing scenarios (e.g. \textit{"think through why you will win, but also explicitly consider why your opponent could win,"}) significantly reduced confidence escalation. Overall, confidence increased by only 3.05 percentage points (from 67.03\% to 70.08\%), a marked improvement over the ~10-11\% average escalation in other experiments (details in Appendix~\ref{appendix:self_redteam_ablation}). These findings suggest that prompting models to consider alternative outcomes is a promising, though not universally effective, mitigation for overconfidence.

% ---- New concise mention of Redteam v2 ----
\paragraph{Redteam v2 (RPT prompt).} We also evaluated a variant that leverages the \emph{Reasoning through Perspective Transition} (RPT) prompt from \citet{wang2025perspectivetransitionlargelanguage}.  This ``Redteam v2'' setup kept the average \emph{opening} confidence essentially unchanged (71.0\%) but further dampened escalation: confidence rose by only 5.7 percentage points from opening to closing, roughly 4 pp lower than the standard debate baseline.  The smaller improvement compared to Redteam v1 likely reflects the fact that the RPT prompt was not tuned for a multi-round debate context.  Full per-model results are provided in Appendix~\ref{tab:self_redteam_v2}.
